...
//
// RCS ID
// $Id: SmcJavaGenerator.java,v 1.13 2013/09/02 14:45:58 cwrapp Exp $
//
// CHANGE LOG
 ...
 */
public final class SmcRxJavaGenerator
...
//
   ...
   */
  public SmcRxJavaGenerator(final SmcOptions options) {
    super(options, "java");
  ...
  //
  /**
   * Emits Java code for the finite state machine context class.
   *
  ...
  public void visit(SmcFSM fsm) {
    ...
    _source.println("import java.util.concurrent.atomic.AtomicBoolean;");
    ...
    for (SmcTransition transition : transitions) {
      String parameter = String.join(", ", transition.getParameters().stream().map(p -> p.getType() + " " + p.getName())
    ...
    Map<String, SmcGuard> guards = new TreeMap<>();
    Map<String, Map.Entry<SmcAction, SmcTransition>> actions = new TreeMap<>();
    for (SmcMap map : fsm.getMaps()) {
      for (SmcState state : map.getStates()) {
        if (state.getEntryActions() != null) {
          for (SmcAction ea : state.getEntryActions()) {
            String name = map.getName() + "_" + state.getName() + "_Entry";
            actions.put(name,
              new AbstractMap.SimpleEntry<>(ea, new SmcTransition(name, Arrays.asList(), state.getLineNumber(), state)));
          }
        }

        if (state.getExitActions() != null) {
          for (SmcAction ea : state.getExitActions()) {
            String name = map.getName() + "_" + state.getName() + "_Exit";
            actions.put(name,
              new AbstractMap.SimpleEntry<>(ea, new SmcTransition(name, Arrays.asList(), state.getLineNumber(), state)));
          }
        }
        ...
        for (SmcTransition transition : state.getTransitions()) {
          String parameter = String.join(", ", transition.getParameters().stream().map(p -> p.getType())
            .collect(Collectors.toList()));

          for (SmcGuard guard : transition.getGuards()) {
            Function<String, String> extractFName = block -> Arrays.asList(block.split("\\W")).stream().
              filter(s -> s.length() > 0 && !"owner".equals(s)).
              findFirst().orElse("");

            String fName = extractFName.apply(guard.getCondition());
            if (fName.length() > 0) {
              guards.put(fName + "_" + parameter, guard);
            }

            for (SmcAction action : guard.getActions()) {
              String parameterWithValue = String.join("_",
                action.getArguments().stream().
                map(name -> transition.getParameters().stream().filter(p -> p.getName().equals(name)).findFirst().orElse(null)).
                filter(f -> f != null).
                map(p -> p.getType()).
                collect(Collectors.toList())
              );
              actions.put(action.getName() + parameterWithValue, new AbstractMap.SimpleEntry<>(action, transition));
            }
          }
        }
      }
    ...
    // Guard functions
    guards.forEach((k, v) -> {
      String parameter = String.join(", ", v.getTransition().getParameters().stream().map(p -> p.getType() + " " + p.getName())
        .collect(Collectors.toList()));
      ...
      if (functionName.length() > 0) {
        _source.println(intent + "@FunctionalInterface");
        _source.println(intent + "public interface " + functionName + " {");
        _source.println(intent + intent + "Observable<Boolean> apply(" + parameter + ");");
        _source.println(intent + "}");
        _source.println(intent + "public " + functionName + " " + functionName + ";");
      }
    });

    // Actions functions
    actions.forEach((k, e) -> {
      SmcAction action = e.getKey();
      SmcTransition transition = e.getValue();

      String variableTypes = String.join("_",
        action.getArguments().stream().
        map(name -> transition.getParameters().stream().filter(p -> p.getName().equals(name)).findFirst().orElse(null)).
        filter(f -> f != null).
        map(p -> p.getType()).
    ...
    for (SmcTransition trans : transitions) {
      ...
      if (transName.equals("Default") == false) {
        ...
        _source.println("            return Default(context);");
  ...
  private void appendFunctionalInterface() {
...
//         _source.println();
  ...
  public void visit(SmcTransition transition) {
    ...
    for (git = guards.iterator(), _guardIndex = 0, _guardCount = guards.size();
      git.hasNext() == true;
      ++_guardIndex) {
      guard = git.next();
    ...
    if (_guardIndex > 0 && nullCondition == false) {
      ...
      _source.print(_indent + "    ");
      _source.println("Observable.just(Boolean.TRUE).");
      _source.print(_indent + "    ");
      _source.println("last().");
      _source.print(_indent + "    ");
      _source.println("filter(b -> !transitionCompleted.get() && b).");
      _source.print(_indent + "    ");
      _source.println("flatMap(v -> ");
      ...
      _source.print(_indent + "    ");
      _source.println("transitions.add(transition);");
  ...
  public void visit(SmcGuard guard) {
    ...
    if (condition != null && condition.length() > 0) {
      functionName = Arrays.asList(condition.split("\\W")).stream().
        filter(s -> s.length() > 0 && !"owner".equals(s)).
        findFirst().orElse("");

      parameter = String.join(", ", guard.getTransition().getParameters().stream().map(p -> p.getName())
        .collect(Collectors.toList()));
    }
    ...
    if (_guardCount > 1) {
      ...
      if (_guardIndex == 0 && condition.length() > 0) {
        ...
        _source.print(_indent + "    ");
        _source.print("context.");
        _source.println(functionName + "." + condition.replaceFirst(functionName, "apply") + ".");
    ...
    if (hasActions == false) {
      ...
      // current state.
      _source.print(indent2);
      _source.println("context.rxClearState(),");
    ...
    // Print the rxSetState() call, if necessary. Do NOT
    ...
    if (transType == TransType.TRANS_SET
      ...
      _source.print("context.rxSetState(");
      _source.print(fqEndStateName);
      _source.println("),");
      ...
      _source.print(indent3);
      _source.print("context.rxPushState(");
      _source.print(pushStateName);
      _source.println("),");
    } else if (transType == TransType.TRANS_POP) {
      _source.print(indent3);
      _source.println("context.rxPopState(),");
    }
   ...
   *
   * @param parameter emits Java code for this transition parameter.
   */
